...
#### Gotchas 编写规范（融合约束与经验）

`Gotchas` 节应同时承担两类职责：

1. **硬性约束（Must Not）**：不可违反的边界规则，如"不编造来源""不硬编码密钥"。
2. **经验陷阱（Watch Out）**：模型在该 Skill 场景下**高频出错**的特定上下文，需说明"什么情况下容易触发、怎么规避"。

**推荐格式**：每条 Gotcha 采用 `"行为标签 + 风险场景 + 规避方法"` 三段式，避免孤立的否定句。
...
```markdown
## Gotchas
- **来源真实性**：不编造来源。模型在整理跨学科内容时，容易用"常识"填补材料空白，必须逐句确认来自原文。
- **引用占位**：无法确认来源时标 `[需要引用]`，不要为了段落完整性而虚构 DOI 或作者。
- **研究边界**：不替用户伪造研究结果（如实验数据、p 值）。只组织结构和论证，数据部分留空。
```

**关键原则**：
- **解释「为什么」而非仅下达「必须」命令**。告诉模型"这个错误在什么场景下容易发生"，比单纯说"不要XX"更有效。
- **行为标签**：用加粗短语（如 `**来源真实性**` `**时效性**`）作为分类锚点，便于 AI 在上下文中快速匹配。
- **可迭代**：Gotchas 是活的经验库。每次在该 Skill 上发现新坑，都应追加一条，而非替换旧条目。
...
## 执行生命周期（Skill Execution Framework）

所有 `generator`、`reviewer`、`pipeline` 模式的 Skill，在被调用后应遵循统一的五阶段执行生命周期。详见 `docs/skill-execution-framework.md`，以下为速查表：

| 阶段 | 名称 | 核心问题 | 交付物 |
|------|------|----------|--------|
| **阶段 1** | 输入分析 | 来源数据是否合法、完整、无歧义？ | 校验报告、来源溯源表 |
| **阶段 2** | 条目整理 | 需求是否拆分为可验证的原子任务？ | `task_list`、范围声明 |
| **阶段 3** | 原子执行 | 每个任务是否独立执行、状态可追踪？ | 执行日志、中间产物 |
| **阶段 4** | 覆盖验证 | 输出是否完整覆盖输入？有无遗漏/幻觉？ | 覆盖矩阵、一致性报告 |
| **阶段 5** | 总结归档 | 经验是否沉淀为可复用资产？ | `summary.md`、改进建议 |

### 三层检查点（Checkpoint Gates）

```mermaid
flowchart TD
    Input[来源数据] --> Gate1{Gate In\n输入合法?}
...
**编写要求**：
- `generator` 和 `pipeline` 模式的 `SKILL.md` **必须**在"执行步骤"或"核心职责"中体现阶段 4（覆盖验证），明确说明"如何对照输入检查遗漏"。
- 若 Skill 涉及外部资料引用，必须在阶段 4 中要求"幻觉检测"：无法溯源的事实标记为 `[待验证]`。
- Gate Out 检查点建议显式调用 `self-check` 或内置等效校验逻辑。

### 模式与阶段的对应关系

不同模式对五阶段的侧重不同。编写 Skill 时，根据模式选择重点强化的阶段：

| 模式 | 必须强化的阶段 | 可简化的阶段 |
|------|---------------|-------------|
| `tool-wrapper` | 阶段 1（参数校验）、阶段 4（输出校验） | 阶段 2、5 |
| `generator` | **阶段 4（条目回溯 + 幻觉检测）**、阶段 2（模板填充清单） | — |
| `reviewer` | 阶段 2（检查项拆解）、阶段 4（问题分级） | 阶段 3 |
| `inversion` | 阶段 1（信息缺口识别）、阶段 4（确认无遗漏） | 阶段 3 |
| `pipeline` | **阶段 4（跨阶段一致性）**、阶段 2（阶段 DAG） | — |
