...
> ⚠️ **Kimi Code 特殊限制**：Kimi Code 的 skill 加载器对 Frontmatter 采用**严格白名单校验**，仅识别 `name` 和 `description` 两个字段。如果 `SKILL.md` 中包含 `metadata`、`license`、`compatibility` 等扩展字段，Kimi Code 会**主动报错并拒绝加载**该 skill。
>
> 本项目采用**双轨制**解决跨平台兼容问题：
> - `SKILL.md` → 仅保留 `name` + `description`（Kimi 严格版）
> - `meta.json` → 存放 `tags`、`platforms`、`version` 等全量元数据（Claude / 外部工具可用）
...
由于 Kimi Code 的 Frontmatter 严格白名单限制，本项目将扩展元数据从 `SKILL.md` 移至同目录的 `meta.json`：
...
| 文件 | 用途 | 读取方 |
|------|------|--------|
| `SKILL.md` Frontmatter | Skill 匹配触发 | 所有平台（Kimi 仅限 `name` + `description`） |
| `meta.json` | 扩展元数据、版本管理、标签检索 | 外部工具、索引生成器、Claude 生态工具 |

### 平台兼容速查

| 平台 | Frontmatter 支持 | `meta.json` 支持 |
|------|-----------------|------------------|
| **Kimi Code** | ⚠️ 仅 `name` + `description` | 不读取，无影响 |
| **Claude Code** | `name`、`description`、`metadata`、`license`、`compatibility`、`allowed-tools` | 不读取，可由外部工具合并 |
| **Cursor** | 转 `.mdc` 格式时由转换脚本映射 | 不读取，可由转换脚本引用 |
| **Codex / Gemini** | 宽松解析，通常保留未知字段 | 不读取，无影响 |
