...
| 模块化处理 | 基于详细需求中的模块拆分（`feature-XX-{模块名}/`），逐模块独立输出 |
...
| `api-spec.md` | 模块对外暴露的接口定义（含请求/响应字段、错误码、权限） | `specs/feature-XX-{模块名}/` |
| `db-schema.md` | 数据表结构、DDL 语句、索引建议、缓存策略、连接池配置 | `specs/feature-XX-{模块名}/` |
| `state-machine.md` | 模块内部状态流转图（Mermaid）、状态转换条件、异常分支 | `specs/feature-XX-{模块名}/` |
| `test-plan.md` | 单元测试用例设计、集成测试场景、边界条件覆盖、Mock 策略 | `specs/feature-XX-{模块名}/` |
...
sequenceDiagram
    ...
    loop 逐模块串行生成（5 文件/模块）
        ...
        DD->>DR: 保存到 specs/feature-XX-{模块名}/
    end
...
2. **模块识别**：自动解析 `03-functional-structure.md` 或扫描 `feature-*/` 目录获取模块清单
