...
4. **禁止内部引号**：节点文本中不出现中文双引号 `""` 或英文双引号 `"`（除非整个节点已被双引号包裹）；如需强调，改用「」、《》、' 或省略
5. **拒绝 HTML 实体**：不写 `&#123;` / `&#125;`，直接写 `{` / `}` 并用引号包裹
6. **换行符统一**：节点内换行使用 `<br>` 或 `\n`，禁止 `<br/>` / `<br />`
7. **子图闭合**：每个 `subgraph` 都有对应的 `end`
8. **特殊字符保护**：节点文本含 `&` `<` `>` 时必须加双引号包裹
9. **括号嵌套处理**：节点文本含 `()` 时必须整体加双引号包裹，如 `["Node (with parens)"]`
10. **无引号叙述优先**：节点文本中不含内部双引号，语义简洁；如需强调改用 `「」`、《》、' 或省略
...
每个 `flowchart` 必须包含显式的开始节点和结束节点，中间节点禁止悬空：

- **开始节点**：使用 `([开始])` 或语义化名如 `([用户访问])`
- **结束节点**：使用 `([结束])` 或语义化名如 `([流程完成])`
- **中间节点悬空禁止**：任何非 Start/End 的中间节点必须有入边和出边（终止节点除外）。孤立的、无上下游连接的节点必须删除或补充关系

### 规则 13：箭头标签语义化

决策节点的出口标签禁止使用简单的"是/否/yes/no"：

- ❌ `B -->|否| C`、`B -->|yes| D`
- ✅ `B -->|校验失败| C`、`B -->|已选择| D`、`B -->|未授权| E`

循环回退、异常分支等回流路径的标签必须使用**业务动词或状态描述**，让阅读者无需回看决策节点文本即可理解路径含义。

### 规则 14：节点文本书写规范

1. **优先无引号叙述**：节点文本保持简洁自然，如 `A[用户登录]` 而非 `A[用户点击"登录"]`
2. **节点内部禁用双引号**：`[...]` / `([...])` / `{...}` 内的文本不出现英文双引号 `"` 或中文双引号 `""`
   - 如需强调，改用 `「」`、《》、`'` 或直接省略
   - ❌ `[提示"密码错误"]`；✅ `[提示密码错误]` 或 `[提示'密码错误']`
3. **必须保留引号时的包裹规则**：若节点文本语义上必须包含引号字符，用 `id["..."]` 将整个文本包裹，**内部双引号改为单引号**
   - 示例：`A["系统提示'操作失败'"]`
...
- [ ] **特殊字符已保护**（含 `&` `<` `>` 的节点已加双引号）
- [ ] **含 `()` 嵌套冲突的节点已处理**（节点文本含圆括号时必须用双引号包裹整个节点，如 `["Node (with parens)"]`）
