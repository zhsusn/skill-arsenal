...
> **粗粒度并行原则**：前期阶段（序号 1-4）不要求严格串行。概要需求（序号 1）冻结后，详细需求（序号 2）和概要设计（序号 3）可**同时启动**；详细设计（序号 4）可在对应模块的详细需求评审通过后即启动，无需等待全部 PRD-001~00N 完成。
>
...
6. **🚪 禁止跳过人工闸门（V2.1 新增）**：
   ...
   - Gate 2.5 未签字（`human_status.gate2_5 != passed`）：禁止进入概要设计阶段
