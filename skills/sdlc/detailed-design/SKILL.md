...
|---|---|---|---|
| `high-level-design` | `high-level-design/01-05.md`（6 个主题文件） | 架构约束、技术选型、全局状态机 | **必须** |
...
对每个 `feature-XX-{模块名}` 独立生成 1 个主题文件 `module-design.md`，内部包含 5 个原子章节：

#### ## 1. 模块架构与组件设计（原 design.md）
...
**Mermaid 工程规范约束**：
- 所有图表必须遵循 `mermaid-diagrams` skill 的工程化规范规则和质量检查清单
- **模块依赖图（`flowchart`）**：节点 ID 使用语义化前缀（`Pg_`/`Dec_`/`St_`/`Proc_`/`Svc_`）；节点 > 10 时用 `subgraph` 按分层分组；回流线（如循环依赖、重试路径）用 `-.->` 虚线；样式集中声明
- **时序图（`sequenceDiagram`）**：参与者在顶部显式声明；请求用 `->>`、响应用 `-->>`、异步用 `-x` 或 `--x`；长操作使用激活条（`+`/`-`）
- **类图（`classDiagram`）**：关系箭头方向明确；避免节点文本含 `<` `>` `&` 等特殊字符未保护
- **状态图（`stateDiagram-v2`）**：每个子状态块 `{ ... }` 必须闭合；状态转换标注触发条件，含特殊字符时加双引号
- **通用**：换行符统一为 `<br>`，禁止 `<br/>`；禁止节点文本直接写 URL；禁止纯中文或纯数字节点 ID
...
每个模块只生成 1 个文件：
- **`module-design.md`**：合并 design + api-spec + db-schema + state-machine + test-plan

同时保存以下两类全局文件到 `openspec/changes/{变更名}/detailed-design/` 根目录：
...
> **合并边界**：`module-design.md` 内的 5 个原子章节红线独立生效。禁止在章节间混入不相关内容。
...
| 产出物 | 内容说明 | 保存路径 |
|--------|----------|----------|
| `module-design.md` | 模块内部架构、接口定义、数据表结构、状态机、测试策略 | `detailed-design/feature-XX-{模块名}/` |
...
- **Mermaid 语法陷阱**：`classDiagram` 中类名含泛型（如 `List<User>`）时必须加双引号；`stateDiagram-v2` 中子状态块忘记闭合 `end` 会导致后续所有状态被吞入子图；`sequenceDiagram` 中未声明的参与者会导致渲染失败。生成后必须执行 `mermaid-diagrams` skill 的致命规则检查清单。
